...
def _exec_summary(project, r, status_text) -> List[str]:
    npv = float(r.get("npv", 0))
    wacc = float(r.get("wacc", 0))
    inf = float(r.get("inflation_used_pct", 0))
    be = float(r.get("breakeven_revenue", 0))
    return [
        "EXECUTIVE SUMMARY",
        "",
        ...
        "NPV: USD %s | IRR: %s | MIRR: %s | WACC: %s" % (
            _usd(npv), _rate(r.get("irr", 0)), _rate(r.get("mirr", 0)),
            _rate(wacc)),
        "Payback: %s years | PI: %.2f | EAA: USD %s" % (
            _payback(r.get("payback", float("inf"))), r.get("pi", 0),
            _usd(r.get("eaa", 0))),
        "Break-even revenue: USD %s" % _usd(be),
        "Reason: %s" % latin1(r.get("reason", "")) if r.get("reason") else "",
        "",
        "WHAT TO MONITOR (from the same model results):",
        "  * Inflation: the model escalated costs at %s." % _pct(inf),
        "  * Foreign exchange: %s%% of capex is imported hard currency - ZiG/USD moves change cost." % int(project.imported_equipment_pct),
        "  * Interest rate: WACC %s feeds directly into NPV, IRR and PI." % _rate(wacc),
        "  * Cost overrun / delay: contractor %s, %d procurement-delay days, %d change orders." % (
            project.contractor_type, int(project.procurement_delay_days), int(project.num_change_orders)),
        "  * Revenue vs cost growth: revenue +%s%% vs operating-cost +%s%%." % (
...
def _findings_text(project, r, status_text) -> List[str]:
    ...
    monitor = (
        "Key management monitoring points: (1) inflation - the model lived with "
        "%s inflation; (2) the ZiG/USD exchange rate - %s%% of the capital is "
        "imported; (3) interest rates - the WACC is %s; (4) the project schedule "
        "and cost-overrun probability; and (5) material prices. Re-run the macro "
        "stress lab before the final investment decision."
        % (_pct(float(r.get("inflation_used_pct", 0))),
           int(project.imported_equipment_pct), _rate(float(r.get("wacc", 0)))))
    return [finding, "", monitor, "", "Generated by the Vision 2030 CapEx AI Agent."]


def _vision_2030(project) -> List[str]:
    from core.vision_2030 import project_alignment
    al = project_alignment(project)
    out = ["VISION 2030 ALIGNMENT", ""]
    out.append("Underlying framework: %s" % al["pillar"])
    out.append("")
    out.append("Estimated alignment: %d/100 (%s alignment)" % (al["score"], al["band"]))
    out.append("")
    for i, g in enumerate(al["goals"], 1):
        out.append("%d. %s - %s (weight %d/100)" % (i, g["code"], g["name"], g["weight"]))
    out.append("")
    for g in al["goals"]:
        out.append("- %s: %s" % (g["code"], g["rationale"]))
    out.append("")
    out.append(al["summary"])
    out.append("")
    out.append("Alignment scores are deterministic AI judgement based on the "
               "project sector and type. They are decision-support estimates, "
               "not official government endorsements.")
    return out
...
_ALL_SECTIONS = [
    ...
    "Data Limitations", "Vision 2030 Alignment", "Final Findings",
]
...
def _build_items(project, macro, result, sections):
    ...
    builders = {
        ...
        "Vision 2030 Alignment": lambda: _vision_2030(project),
...
def generate_text_report(project: ProjectInput, macro: MacroContext,
    ...
    blocks = ["=" * 78, latin1("VISION 2030 CAPEX AI AGENT - MANAGEMENT REPORT"),
...
class _ReportPDF:
    ...
    def _header_every_page(self):
        ...
        self.pdf.cell(0, 8, latin1("VISION 2030 CAPEX AI AGENT - MANAGEMENT REPORT"),
